using SkullKingCore.Network.TCP.Networking;
using System.Net.Sockets;

namespace SkullKingCore.Network.TCP.Rpc
{
    public sealed class TcpRpcConnection : IAsyncDisposable
    {
        private readonly TcpClient _client;
        private readonly NetworkStream _stream;
        private readonly CancellationTokenSource _cts = new();

        private TcpRpcConnection(TcpClient client)
        {
            _client = client;
            _stream = client.GetStream();
        }

        public static async Task<TcpRpcConnection> ConnectAsync(string host, int port, CancellationToken ct)
        {
            var tcp = new TcpClient();
            await tcp.ConnectAsync(host, port, ct).ConfigureAwait(false);
            return new TcpRpcConnection(tcp);
        }

        public static Task<TcpRpcConnection> FromAcceptedAsync(TcpClient accepted, CancellationToken ct)
            => Task.FromResult(new TcpRpcConnection(accepted));
