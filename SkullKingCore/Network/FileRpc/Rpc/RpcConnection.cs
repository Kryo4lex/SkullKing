...
namespace SkullKingCore.Network.FileRpc.Rpc
{
    ...
    public sealed class RpcConnection : IAsyncDisposable
    {
        ...
        private RpcConnection(string clientId, Role role, AppendOnlyFileChannel incoming, AppendOnlyFileChannel outgoing, CancellationToken externalCt)
        {
            _clientId = clientId;
            _role = role;
            _incoming = incoming;
        ...
        public static Task<RpcConnection> ConnectAsync(string clientId, string folder, CancellationToken ct)
        {
            ...
            var conn = new RpcConnection(clientId, Role.Client, incoming, outgoing, ct);
            return Task.FromResult(conn);
        }

        /// <summary>SERVER entry: binds to an already discovered clientId at folder.</summary>
        public static Task<RpcConnection> FromAcceptedAsync(string folder, string clientId, CancellationToken ct)
        {
            if (string.IsNullOrWhiteSpace(clientId)) throw new ArgumentNullException(nameof(clientId));
            if (string.IsNullOrWhiteSpace(folder)) throw new ArgumentNullException(nameof(folder));
            ...
            var conn = new RpcConnection(clientId, Role.Server, incoming, outgoing, ct);
            return Task.FromResult(conn);
        }
