using SkullKingCore.Logging;
using SkullKingCore.Utility;
using SkullKingCore.Test;

namespace SkullKingSandboxConsole.Commands
{
    public static class CommandTrickSimulation
    {

        public static void Run()
        {

            bool seedGameRunning = true;

            int playerCount;
            int round;

            if (!UserConsoleIO.TryReadInt("Enter Player Count:", out playerCount, Settings.MinPlayerCount, Settings.MaxPlayerCount))
            {
                Logger.Instance.WriteToConsoleAndLog("Cancelled.\n");
                return;
            }

            if (!UserConsoleIO.TryReadInt("Enter Round:", out round, 1, Settings.MaxRounds))
            {
                Logger.Instance.WriteToConsoleAndLog("Cancelled.\n");
                return;
            }

            int seed = 0;

            while (seedGameRunning)
            {

                Console.Clear();

                TrickSimulation trickSimulation = new TrickSimulation(playerCount, round, seed);

                trickSimulation.Play();

                var ch = Console.ReadKey(false).Key;

                switch (ch)
                {
                    case ConsoleKey.E:
                        seedGameRunning = false;
                        return;
                    case ConsoleKey.LeftArrow:
                        seed--;
                        break;
                    case ConsoleKey.RightArrow:
                        seed++;
                        break;
                }

            }

        }

    }
}
